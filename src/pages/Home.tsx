 ...
 *   EarlyRetireSim -> the app's display name
 *   월급, 지출, 저축률을 입력하면 몇 살에 조기은퇴(FIRE)가 가능한지 계산해주는 시뮬레이터    -> the one-line description
 */
...
export default function Home() {
  ...
  return (
    <ScreenScaffold
      top={<Top title={<Top.TitleParagraph>EarlyRetireSim</Top.TitleParagraph>} />}
    >
      {/* 시각 앵커: 헤드라인 + 카드 내 진입 버튼(부유 금지, display="block" 전체폭).
          데이터 앱이면 value를 <Amount typography="t1" />(핵심 숫자)로 교체하라. */}
      <SummaryHero
        label="EarlyRetireSim"
        value={<Paragraph.Text typography="t2">월급, 지출, 저축률을 입력하면 몇 살에 조기은퇴(FIRE)가 가능한지 계산해주는 시뮬레이터</Paragraph.Text>}
